...
# TODO: Extend +,- to for dissimilar spaces?
...
function ==(X::MultiMatrix, Y::MultiMatrix)
	...
	# 		ijX = cat(invpermute(Tuple(ii), iX), pjx)
	# 		ijY = cat(invpermute(Tuple(ii), iY), pjy)
	# 		if X[ijX...] != Y[ijY...]
	# 			return false
	# 		end
	# 	end
...
"""
chk_square(M::MultiMatrix) = laxes(M) == raxes(M) ? M : error("MultiMatrix is not square.")
chk_square(M::MultiMatrix, d::Dims) = laxes(M, d) == raxes(M, d) ? M : error("MultiMatrix is not square in selected dimensions $d")
...
function tr_dims(M::MultiMatrix, tdims, kdims::Dims)
	...
	lsz = lsize(M)
	rsz = rsize(M)
	R = asdeclared(arraytype(M)){eltype(M),2*K}(undef, lsz[kdims]..., rsz[kdims]...)
...
function +(A::MultiMatrix, B::MultiMatrix)
	...
	axes(A) == axes(B) || error("To add MultiMatrices, they must have the same axes")
...
function -(A::MultiMatrix, B::MultiMatrix)
	...
	axes(A) == axes(B) || error("To subtract MultiMatrices, they must have the same axes")
	return MultiMatrix(A.data - B.data, spaces(A); checkspaces = false)
end
...
function _mult_MA(M::MultiMatrix, A::AbstractArray{TA}) where {TA}
	n = nspaces(M)
	S = spaces(M)
	raxes(M) == axes(A, S) || error("raxes(M) must equal axes(B, spaces(A))")
...
function _mult_AM(A::AbstractArray{TA}, M::MultiMatrix) where {TA}
	n = nspaces(M)
	S = spaces(M)
	laxes(M) == axes(A, S) || error("axes(A, spaces(B)) must equal laxes(B)")
...
"""
function *(A::MultiMatrix, B::MultiMatrix)
	Atype = asdeclared(arraytype(A))
	Btype = asdeclared(arraytype(B))
	...
	if SA == SB
		...
		axes(A, tdimsA) == axes(B, tdimsB) || error("raxes(A) must equal laxes(B) on spaces common to A,B")
...
@generated function get_mult_dims(::Val{SA}, ::Val{SB}) where {SA,SB}
	...
	osa_mask = MVector(filltup(true, nA))
	osb_mask = MVector(filltup(true, nB))
